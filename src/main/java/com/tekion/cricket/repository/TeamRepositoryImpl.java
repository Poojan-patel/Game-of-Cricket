...
public class TeamRepositoryImpl implements TeamRepository{
    ...
    public String getTeamNameByTeamId(String teamId) {
        ...
        String teamName = "";
        try {
            teamName = jdbcTemplate.queryForObject(ReaderUtil.readSqlFromFile("team", "getTeamNameFromTeamId"), String.class, teamId);
...
//        }
    ...
    public String save(Team team){
        String teamId = null;
        try {
            jdbcTemplate.update(ReaderUtil.readSqlFromFile("team", "insertTeamData"), team.getTeamId(), team.getName());
...
//        }
    ...
    public List<Team> findAll(){
        ...
        try{
            teams = jdbcTemplate.query(ReaderUtil.readSqlFromFile("team", "getAll"), BeanPropertyRowMapper.newInstance(Team.class));
        } catch (DataAccessException dae){
            dae.printStackTrace();
        }
//        try{
...
//        Connection con = null;
        AtomicReference<BattingTeam> team = new AtomicReference<>();
        try{
            jdbcTemplate.query(ReaderUtil.readSqlFromFile("team", "fetchTeamScoreFromMatchId"), rs -> {
                team.set(new BattingTeam(rs.getString("name"), rs.getInt("Score"), rs.getInt("Balls"), battingTeamId));
            }, matchId, battingTeamId);
            if(team.get() == null){
                jdbcTemplate.query(ReaderUtil.readSqlFromFile("team", "getTeamNameFromTeamId"), rs -> {
                    team.set(new BattingTeam(rs.getString(1), 0, 0, battingTeamId));
                }, battingTeamId);
            }
        } catch (DataAccessException dae){
            dae.printStackTrace();
...
//        Connection con = null;
        ...
        try {
            jdbcTemplate.query(ReaderUtil.readSqlFromFile("team", "fetchFirstTwoPlayers"), rs -> {
